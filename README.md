...
2026-08-02 起的 `develop` 开发线新增人工作业编辑器、风险确认式继续、召唤物/装置延迟绑定、MAA
兼容战前编队 OCR/F0 头像桥接，并修复部署方向触控越界。相关基础能力已同步到远端；后续目录搜索、
部署后稳定屏障和编辑页实时帧仍只在本地 `develop`，均已通过自动门禁。这些新增能力仍须按实机验收
手册 §5–§10 验证，不能沿用 M8/M9 的历史通过结论。

2026-08-03 的本地构建为非最终部署增加 500ms 权威暂停稳定窗口，避免部署确认后立即发送
`ReleasePause` 被间歇性吞掉；该修复已通过自动门禁，HS-9 F752/F991/F1586 仍待实机复验。

作业编辑页在尺子连接时会独立显示实时绝对帧，以 33ms 轻量刷新；“跟随尺子”只控制时间轴光标，
关闭后仍可观察实时帧。
...
| 7 编辑器、继续与延迟绑定 | 🟡 含地图装置 Skill/location 与本地干员目录搜索，已通过自动门禁；GUI 和真实链路待实机验收 |
| 8 战前编队 OCR/F0 桥接 | 🟡 已实现并通过资源初始化/逻辑测试；真实编队待实机验收 |
| 9 部署方向边界修复 | 🟡 自动回归通过；用户本地 test2 F483 与四方向边缘待实机验收 |
| 10 部署后 Resume 稳定屏障 | 🟡 自动回归通过；HS-9 F752/F991/F1586 待实机复验 |

输入适配、AFA 解析和动作确认测试已通过；`cargo test --workspace` 当前共 259 项，clippy、release 构建、
...
1. 填写作业标题和关卡标识；在编队栏输入干员名称的一部分即可搜索本地 MAA 目录，例如输入
   `安` 可选择“安赛尔”“安洁莉娜”等候选。也可保留手动输入，用于自定义名称。
2. 把光标设到目标帧，添加部署、技能/装置、撤退或注释动作。
3. 在右侧属性面板填写目标、格子与朝向；召唤物或需要命名的装置加入独立目标名单，不加入开局编队。
4. 处理底部错误诊断。无效草稿仍可保存，但不能交给复刻模式运行。
5. 保存后切到“复刻执行”。通过严格校验的文档会沿用原有 AFA/尺子复刻链路。

召唤物第一次在目标帧进入部署栏时，程序保持暂停并优先用全局头像档案匹配；无法唯一匹配才要求
人工绑定一次。绑定期间不发送游戏输入，切回游戏后必须由新的同目标帧尺子样本复核才能部署。

复刻执行页还提供显式“扫描编队”。请在战前编队确认界面点击，核对 OCR 姓名和头像后确认；进入
关卡并停在 F0 时，程序会用职业约束、多尺度头像匹配把战前大头像桥接到部署栏卡片。只有唯一高分
匹配才保存，失败或歧义仍走人工绑定。OCR 直接兼容 MAA `resource/PaddleOCR/rec`，无需训练模型、
安装 OCR 或加载 `MaaCore.dll`；扫描本身只截图，不调用 AFA 或输入接口。

地图装置技能使用与 MAA 相同的 `Skill + location` 语义。在编辑器点击“+ 技能 / 装置”，直接填写
逻辑格子或在地图上点选；名称可以留空，也可以先把“留声机”等名称加入“召唤物 / 装置目标”后从
下拉菜单选择。显式格子优先于名称，装置不进入部署栏绑定：
...
普通干员技能仍可不填格子，运行时按此前部署记录定位；属性面板的“清除格子”可从装置模式切回
按名称定位。
...
1. 可选：在战前编队确认界面点击“扫描编队”，核对并确认识别结果；不扫描仍可使用原有绑定。
2. 用户进入关卡，等待 AFA 自动暂停。
3. 确认尺子显示带 `frame_id` 的可信战斗内 `1x_paused` 第 0 帧，点击“开始复刻”。
4. 复刻器保持显示，等待用户手动点击游戏窗口；不会自动抢游戏焦点。
5. 焦点后的新尺子样本决定绑定模式：paused 可人工绑定，running 只允许已有档案自动恢复。
6. 编队准备完成后再次等待用户把游戏切回前台，以最终焦点后的新可信样本初始化 Machine。
