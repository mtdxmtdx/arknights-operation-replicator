...
1. 启动 `repl-app.exe`，在首次向导中选择有效的 MAA `resource` 目录。
2. 打开“作业编辑”页，新建作业或打开已有 JSON。
3. 填写标题和关卡标识。关卡标识匹配成功后，编辑器会显示逻辑地图；匹配失败时仍可手填格子，
   但作业不能进入正常复刻。
4. 如需边看实机边打轴，启动并连接尺子。编辑页会显示当前绝对帧；“跟随尺子”只控制时间轴光标，
   不会向游戏发送任何输入。
