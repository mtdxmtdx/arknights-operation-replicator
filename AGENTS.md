...
- 第 0 帧暂停可能冻结标题/费用条渐显，恢复后第一条可信读数实测可到第 33 帧；正式验收作业首动作
  至少第 60 帧。仓库 `sample-job.json` 的 10/40/60 序列只用于早帧时序回归，此类巡航越过与
  `initial_gap_ms` 无关。
...
2026-07-29 实机通过；M9 仍必须使用第 0 帧普通精确运行完成录像与 CSV 10/10，风险继续不得计入。
